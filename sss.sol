...
contract Sss is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 50000 ether;
    // Initial distribution for the day 2-5 SSS-WFTM LP -> SSS pool
    // uint256 public constant INITIAL_SSS_POOL_DISTRIBUTION = 123990 ether;
    ...
    uint256 public burnThreshold = 4.40e15;
    ...
    // Tax Tiers
    uint256[] public taxTiersTwaps = [0, 3.20e15, 3.40e15, 3.60e15, 3.80e15, 3.88e15, 3.96e15, 4e15, 4.04e15, 4.12e15, 4.20e15, 4.40e15, 4.60e15, 4.80e15];
    uint256[] public taxTiersRates = [2600, 2500, 2400, 2300, 2200, 2100, 2000, 1800, 1600, 1500, 1000, 400, 200, 100];
    ...
    /**
     * @notice Constructs the SSS ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("SSS", "SSS") {
        // Mints 1 SSS to contract creator for initial pool setup
    ...
    function _getSssPrice() internal view returns (uint256 _sssPrice) {
        try IOracle(sssOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Sss: failed to fetch SSS price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _sssPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_sssPrice >= taxTiersTwaps[tierId]) {
    ...
    function setSssOracle(address _sssOracle) public onlyOperatorOrTaxOffice {
        require(_sssOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints SSS to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of SSS to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentSssPrice = _getSssPrice();
            currentTaxRate = _updateTaxRate(currentSssPrice);
            if (currentSssPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // _mint(_sssPool, INITIAL_SSS_POOL_DISTRIBUTION);
